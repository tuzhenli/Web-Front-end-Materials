...
通常SDK是由专业性质的公司提供专业服务的集合，比如提供安卓开发工具、或者基于硬件开发的服务等。也有针对某项软件功能的SDK，
如推送技术、图像识别技术、移动支付技术等，同时资源优势类的公司也提供资源共享的SDK，如一些广告SDK提供盈利渠道，分发SDK提
供产品下载渠道。
zepto和jquery的区别?
    jQuery 由于强大的生命力基本上是一个事实标准，所以大部分工具 lib 在 DOM 操作、动画等功能上或多或少都会是 jQuery-like 的。
Zepto 的 API 就是完全兼容 jQuery 的，功能上 Zepto 是 jQuery 的子集，定位上 jQuery 桌面为主，Zepto 则从一开始就定位移动设
备，所以体积方面有一定优势。也就是说如果你只用了 jQuery 很少且都是核心的功能，比如选择器等，那么可以在不改动任何业务代码
的情况下把 lib 切换到 Zepto——其实就是把 $ 对象换掉了。
    B/S技术指browser/server指浏览器和服务器端
----------------------------------fe3 debug方法、数组和字符串--------------------------------------------
浮点数不能直接比较，需要借助函数：
    var floatEqual = function   (a, b) {
        var delta = 0.0001;
        return Math.abs(a - b) < delta;    //Math.abs（-1）返回绝对值1  Math.floor（3.9）向下取整3 Math.ceil（4.1）向上取整5
    }
    var sin30 = Math.sin(30 * (Math.PI / 180))
    console.log("0.5", sin30, floatEqual(0.5, sin30))  //  0.5 0.49999999999999994 true
log函数：
 ...
 模板字符串` ${变量} `是ES6的
数组：
    向已经存在的 array 中添加新元素，可以用列表的 push 函数往列表末尾插入一个元素，如：array.push('新元素')
字符串：
    字符串的includes(包含)
 log('impossible'.includes('possible'))
    // 字符串相当于一个 array，可以用数字下标访问
    // 看例子，看结果
    // s 的长度为 7，但是下标是从 0 开始的，所以最大下标是 6
 var s = 'iamgood'
 log(s[0]) //i
 log(s[1]) //a
 ...
 // 我们只讨论第一种情况
 // 需要注意的是, 递归是一个比较难以理解的概念，看不懂, 不理解, 太正常了
 // 用递归求阶乘
 // 阶乘的定义如下
 // n! = n * (n-1)!
 ...
 // 右边的是 value(值), 可以是任意类型, 包括 number string bool array object 等
 ...
 // 修改已有的元素
 gua['name'] = 'gua'
 // 删除元素, 使用 delete 语句, 这个功能很少用, 看看就好
 delete gua.gender
 log('object删除', gua)
 ----------------------------------fe5 html基础、DOM API 和事件--------------------------------------------
 DOM(Document Obejct Model 文档对象模型) 是浏览器对 html 文件的描述方式
 DOM API(Application Program Interface) 是浏览器提供给 JavaScript 操作 html 页面内元素的方式
    简而言之, 浏览器提供了一些内置函数来让我们操作页面(增删改查)
 增加元素：先用 document.createElement 函数创建一个元素，然后用elements.appendChild 给一个元素添加子元素
 删除元素：
         // 两个方法, 一个是用父节点的 removeChild 函数, 很少这样用
         var pwd = e('#id-input-password')
         form.removeChild(pwd)
         // 经常用的方法是直接调用 元素的 remove 函数--------------------常用方式
         //button.remove()
 修改元素：var button = e('button')，用 innerHTML 设置属性，即 button.innerHTML = '注册用户'
...
//清除所有的类class
 ...
 --------------------------------------------------fe6 CSS讲解----------------------------------------------------
 CSS 的使用
     内联(inline style attribute)        强烈不建议使用这种方式
     <head> 标签内的 <style> 标签         这样可以，但不建议使用
     <link> 标签中的外联                  这样是最好的
 三种主要的选择器
     元素选择器
     class 选择器
 ...
 inline元素 只占 content 的尺寸

 overflow属性
     visible 默认
     hidden 隐藏多余元素
     auto 需要的时候加滚动条-------建议用auto
     scroll 就算用不着也会强制加滚动条------很蠢的事情

 background 相关属性和缩写
     常用颜色可以用英语名字, 其他颜色用 十六进制 颜色代码
     R G B
     #233 实际上是 #223333 的缩写
     background-color: #233;
     background-image: url(bg.png);
     background-repeat: no-repeat;
     background-attachment: fixed;  -----背景图片随滚动轴的移动方式, 手机上一般不支持这个属性
     background: #233 url(bg.png) no-repeat fixed;
 ---------------------------------------------fe7 数据类型、函数和todo--------------------------------------------------
    转义符
 // \n     // 表示一个换行符
 // \t     // 表示一个 TAB（制表符）
...
localStorage 是浏览器自带的功能
  ...
  localStorage 只能存储 string 数据，所以没办法直接存储todo里面的数据，要用JSON序列化（JSON.stringify）转化为字符串才行。

 ---------------------------------------------fe9 基础组件、简单动画----------------------------------------------------
                                                none none none
 ---------------------------------------------fe10 作业9讲解、轮播图和定时器--------------------------------------------

 ---------------------------------------------fe11 H5语义化、音频视频、移动开发和桌面开发-------------------------------

 ---------------------------------------------fe12 ES6基础和高级、node和异步编程----------------------------------------

 ---------------------------------------------fe13 http协议、Ajax和BOM--------------------------------------------------

 ---------------------------------------------fe14 作业13讲解、todoAjax事件和Ajax服务器交互--------------------------------------------------

 ---------------------------------------------fe15 类与面向对象基础、this,bind,apply,call函数--------------------------------------------------

 ---------------------------------------------fe16 爬虫基础、爬虫缓存文件、代码结构和封面下载--------------------------------------------------

 ---------------------------------------------fe17 自定义模块和后端基础、express项目结构和常见功能、express动态路由和前端ajax--------------------------------------------------

 ---------------------------------------------fe18 后端博客程序、数据关联和其他--------------------------------------------------

 ---------------------------------------------fe19 同步异步、古代异步解决方案promise、js库lodash和jQuery--------------------------------------------------

 ---------------------------------------------fe20 数据结构和算法分析、四大数据结构、基础结构的实现和面向对象的几个基本概念--------------------------------------------------

 ---------------------------------------------fe21 CSS3新特性、CSS3动画、Canvas库和echart库--------------------------------------------------

 ---------------------------------------------fe22 react 基础、深入和其他--------------------------------------------------

 ---------------------------------------------fe23 面试问题html和css、js基础、js高级和其他--------------------------------------------------

 ---------------------------------------------fe24 简历投递、工作规划与学习进步--------------------------------------------------

 */

class barB extends widget {
...
的扩展语言，如Scss、TypeScript，并将其打包为合适的格式供浏览器使用。
    Webpack的工作方式是：把你的项目当做一个整体，通过一个给定的主文件（如：index.js），Webpack将从这个文件开始找到你的项
